...
rem
rem If no directory is given the deps/omp-stdlib submodule is used, so a
rem plain `extract_all.bat` works after `git submodule update --init`.
setlocal EnableDelayedExpansion
cd /d D:\Work\sampgdk-backup\scripts

if "%~1"=="" (
  set "INC=%~dp0..\deps\omp-stdlib"
  if not exist "!INC!" (
    echo Error: deps\omp-stdlib not found. Run:
    echo   git submodule update --init deps/omp-stdlib
    exit /b 1
  )
) else (
  set "INC=%~1"
)
set OUT=%~dp0..\extracted_natives
